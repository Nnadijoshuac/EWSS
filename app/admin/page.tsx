    ...
    <div className="min-h-screen bg-[#f5f6f2]">
      ...
      <main className="mx-auto max-w-7xl px-4 pb-12 pt-24 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase text-neutral-500">Government console</p>
            <h1 className="mt-2 text-4xl font-black text-neutral-950">Water operations live view.</h1>
          ...
          <span className="rounded-lg bg-neutral-950 px-4 py-3 text-sm font-black text-white">Live data</span>
        </div>
        ...
        <section className="mt-5 rounded-lg border border-black/10 bg-white p-5">
          <div className="mb-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold uppercase text-neutral-500">Supplier verification</p>
              <h2 className="text-2xl font-black text-neutral-950">Pending reviews</h2>
            </div>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {[
              ...
              <div key={item.name} className="rounded-lg border border-black/10 bg-neutral-50 p-4">
                <p className="font-black text-neutral-950">{item.name}</p>
                <p className="mt-1 text-sm font-semibold text-neutral-500">
                  {item.area} - Applied {item.submitted}
                </p>
                <button className="mt-4 rounded-lg bg-neutral-950 px-4 py-2 text-sm font-black text-white">Review</button>
              </div>
            ))}
          </div>
        </section>
...
function Metric({
  ...
  const toneClass = tone === 'red' ? 'text-red-600' : tone === 'green' ? 'text-emerald-700' : 'text-neutral-950';

  return (
    <div className="rounded-lg border border-black/10 bg-white p-4 shadow-sm">
      <p className="text-xs font-bold uppercase text-neutral-500">{label}</p>
      <p className={`mt-2 text-3xl font-black ${toneClass}`}>{value}</p>
      <p className="mt-1 text-xs font-bold text-neutral-500">{note}</p>
    </div>
  );
}
